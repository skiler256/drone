...
USSensor::USSensor(const int pin, gpio& PIN):
pinTrig(pin),
PIN(PIN){}
...
double USSensor::measure(const int pinEcho){
 ...
 while (PIN.read(pinEcho) == 0) {
    if(((std::chrono::high_resolution_clock::now()-t0).count()/1e9)>1) return -1;
 }
 auto t1 = std::chrono::high_resolution_clock::now();

 while (PIN.read(pinEcho) == 1) {
    if(((std::chrono::high_resolution_clock::now()-t1).count()/1e9)>1) return -1;
 }
